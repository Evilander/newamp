...
- **Eviland — a visualizer that reacts to every instrument** — the flagship, and unlike anything else in the Winamp lineage. Where MilkDrop pulses three lumped bands on one shared envelope, Eviland runs a 24-band spectral-flux onset detector so each instrument fires its *own* visual event — the kick blooms a red shockwave from the floor, the snare cracks white off-center, hats sparkle along the top, the lead drifts mid-screen — rendered into a GPU feedback field (WebGL2 float ping-pong) with curl-noise flow, dual-Kawase bloom, and transient-gated chromatic aberration. Underneath it runs a real Navier-Stokes fluid solver (vorticity confinement + Jacobi pressure projection), and the **Liquid** look makes that fluid the picture: every instrument injects its own colored dye — the kick floods up from the floor, the snare jets near-white, hats sparkle the top edge, vocals bloom at their own pitch height — advected by a divergence-free velocity field that goes still in silence and carries stereo pans as visible currents. It has **structural memory** (detects verse/chorus boundaries and visually *rhymes* when a chorus returns) and **anticipation** (locks to tempo and leads the beat). And it **remembers your library** — the first visualizer anywhere with long-term memory of your taste: every song learns a persistent visual plan that survives restarts, evolves a generation at 8/32/96/256 plays (loving a track evolves it early), and seeds brand-new songs from their closest sonic cousin's visual lineage ("Borrowed visual DNA from …"). One tiny database row per track, deterministic seed re-derivation, ancestor seeds preserved forever, one-click purge in Settings. Its Director choreographs **26 distinct morphing looks** built on the same primitives that gave MilkDrop its preset character — q-variable LFOs, per-pixel radial warp profiles, per-channel RGB trail decay, a moving warp centre, video echo, and tear-free field crossfades between sections. Frequency maps to height, stereo to width — it reads like a spectrogram you can feel. Falls back to MilkDrop on machines without WebGL2
...
| Living Tags workshop |
| --- |
| <img src="assets/screenshots/feature-living-tags-workshop.png" alt="NewAmp Living Tags — three-column DSL editor with rule list, expression editor, and live preview" width="420"> |
