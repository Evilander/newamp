...
- **Broad playback support.** Handles MP3, FLAC, OGG/Opus, WAV, M4A/AAC/ALAC, WMA, AIFF, APE, WV, MPC, TTA, MKA, AC3, DTS, DSF, playlists, and ffmpeg fallback formats, with broken-track auto-skip so one bad file does not stall a queue.
...
- **Audio controls and conversion.** 10-band EQ, presets, ReplayGain playback plus selected-track and album loudness analysis, limiter, preamp, crossfade/gapless handoff, output-device selection, in-app left/right speaker test, current-track WAV export, and selected-track batch WAV folder export.
