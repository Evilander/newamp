...
- **Milkdrop visualizer + 18 in-house fullscreen modes** — Butterchurn presets (now sandboxed in their own frame so the main app needs no `unsafe-eval`) plus Particle Flow, beat-locked Tempo Pulse, club-strobe Lattice, neon waves/ribbons, plasma grid, prism bars, burning cloud, aurora, orbital rings and the rest. Real spectral-flux beat detection drives the visuals; an auto hardware tier picks balanced/low so older machines don't lurch
