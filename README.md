...
Also in the stable: real MilkDrop via Butterchurn (sandboxed in its own frame so the app needs no `unsafe-eval`), **Particle Flow** (~140k particles simulated on the GPU via WebGL2 transform feedback through a curl-noise field that bends with the bass), and 24 in-house fullscreen modes. **Auto-Pilot** keeps all of it smooth by *measuring* — every Eviland surface (including the MilkDrop frame) watches its own per-frame render cost and steps resolution, then frame rate, down under sustained load and back up when headroom returns, so a thermal-throttled laptop and a gaming rig both get the best picture they can actually hold. Everything falls back gracefully without WebGL2.
