...
const missing = checkBuildProvenance({ root: smokeRoot, version: '1.0.0' });
assert.equal(missing.ok, false, 'missing provenance should fail');
assert.match(missing.reason, /missing/i);

const planned = buildProvenance({ root: smokeRoot, version: '1.0.0', now: new Date('2026-01-01T00:00:00.000Z') });
...
const written = writeBuildProvenance({ root: smokeRoot, version: '1.0.0', now: new Date('2026-01-01T00:00:00.000Z') });
assert.equal(written.ok, true, written.reason);

await writeFile(join(smokeRoot, 'release', 'NewAmp Portable 1.0.0.exe'), 'portable changed', 'utf8');
const stale = checkBuildProvenance({ root: smokeRoot, version: '1.0.0' });
