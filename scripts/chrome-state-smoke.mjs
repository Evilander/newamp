...
assert.match(firstLaunchSource, /data-newamp-openai-key-prompt/, 'first-launch tutorial should keep the optional-AI line marker');
assert.doesNotMatch(firstLaunchSource, /type="password"/, 'first-launch tutorial must not collect an API key — AI setup lives in Settings');
assert.match(firstLaunchSource, /Start listening/, 'first-launch tutorial primary action should start listening, not configure services');
...
assert.match(fullscreenSource, /VIZ_AUTO_VJ_KEY/, 'fullscreen visualizer should persist Auto VJ mode');
