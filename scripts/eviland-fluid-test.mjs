...
const { fluidForcesFromFrame, dyeDissipationFromFrame, MAX_FLUID_FORCES } = await import(pathToFileURL(resolve('tmp/eviland-fluid-bundle.mjs')).href);
...
{
  const inhaleOnly = fluidForcesFromFrame(frame({ bpm: 120, beatConfidence: 0.9, beatPhase: 0.92 }));
...
const kickForces = fluidForcesFromFrame(kickFrame);
...
const snareForces = fluidForcesFromFrame(snareFrame);
...
const hatDyed = fluidForcesFromFrame(hatFrame).filter(f => f.color && (f.dye ?? 0) > 0 && f.y > 0.7);
...
const vocalForces = fluidForcesFromFrame(vocalFrame);
...
const a = fluidForcesFromFrame(frame({ onsets: [{ band: 5, group: 'other', intensity: 0.8, sharpness: 0.5 }] }));
const b = fluidForcesFromFrame(frame({ onsets: [{ band: 5, group: 'other', intensity: 0.8, sharpness: 0.5 }] }));
...
const f13 = fluidForcesFromFrame(frame({ onsets: [{ band: 13, group: 'vocal', intensity: 1, sharpness: 1 }] }))
  .filter(f => f.color && (f.dye ?? 0) > 0)[0];
const f19 = fluidForcesFromFrame(frame({ onsets: [{ band: 19, group: 'other', intensity: 1, sharpness: 1 }] }))
  .filter(f => f.color && (f.dye ?? 0) > 0)[0];
if (!f13 || !f19) {
