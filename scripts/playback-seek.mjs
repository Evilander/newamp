...
// <audio> scrub probe.
//
// That the served bytes really come from the requested offset is proven in Node
// (rangeBytesOk): a mid-file Range is fetched through the shipped serving code
// and compared byte for byte against the file on disk. An in-page FFT cannot
// prove it — this probe's page is a data: URL, so newamp:// media is
// cross-origin and createMediaElementSource is required to emit silence.
// Fixtures are still frequency-changing tones (440 Hz then 880 Hz) so the
// reported dominantHz is meaningful if this ever runs from a same-origin page.
...
async function run() {
  ...
  for (const c of cases) {
    ...
    // Byte-level proof of the actual regression: ask for a window in the middle of
    // the file and assert the bytes that come back really are the bytes at that
    // offset. The original bug answered every Range with the body from offset 0,
    // which still looks fine to a header-only check if the status is faked.
    // Only meaningful where a concrete file backs the response; the cold
    // transcode path is generated by ffmpeg on the fly and is covered by headers.
    let rangeBytesOk = null;
    const backingFile = mode === 'native' ? c.path : (c.warm ? await peekCachedFlac(c.path) : null);
    if (backingFile && existsSync(backingFile)) {
      const whole = readFileSync(backingFile);
      const start = Math.floor(whole.length / 2);
      const end = Math.min(start + 1023, whole.length - 1);
      const rangeResp = await net.fetch(url, { headers: { Range: `bytes=${start}-${end}` } });
      const served = Buffer.from(await rangeResp.arrayBuffer());
      rangeBytesOk = served.equals(whole.subarray(start, end + 1));
    }
    ...
    results.push({ file: c.name, mode, ...headerProof, rangeBytesOk, headersOk, ...media, ok: !!(headersOk && media.ok) });
    console.error(
...
function seekProbe(url, target) {
  return `
    (async () => {
      ...
      // The graph has to reach the destination or Chromium never pulls it and every
      // bin stays at -Infinity; a zero-gain node keeps the probe silent while still
      // rendering. The context also starts suspended under the autoplay policy.
      ...
      try {
        ...
        // dominantHz is reported for diagnostics only and is NOT asserted: this page
        // is loaded from a data: URL (opaque origin), so the newamp:// media is
        // cross-origin and createMediaElementSource is specified to emit silence.
        // Every bin reads -Infinity here regardless of what was fetched. The
        // "did we get the bytes from the right offset" claim is proven in Node
        // instead, by rangeBytesOk.
        return {
