...
if (!skipPackage) {
  ...
  run('npm', ['run', 'smoke:installed-app'], 'npm run smoke:installed-app');
...
function isExpectedPackagedLaunchWarning(line) {
  return (
    ...
    /\[newamp\] tray icon bounds unavailable/.test(line) ||
    ...
    /Windows notification area did not report tray bounds/.test(line)
  );
}
