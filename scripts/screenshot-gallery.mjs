...
for (const album of albums) await createAlbumFixture(album);
await copyContributedScreenshots();
await writeSmokeSettings();

const result = await runElectronGallery();
...
async function createAlbumFixture(album) {
...
async function copyContributedScreenshots() {
...
function runElectronGallery() {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const timeout = setTimeout(() => {
      finish(new Error(`Screenshot gallery timed out without result marker. stderr:\n${tail(stderr)}`));
    }, 70000);
    ...
    child.on('exit', (code) => {
      if (!settled && code !== 0) {
        finish(new Error(`Electron exited ${code ?? 'without code'} before screenshots completed.\nstderr:\n${tail(stderr)}`));
      }
    });
