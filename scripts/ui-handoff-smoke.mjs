...
const smokeRoot = resolve('tmp', 'ui-handoff-smoke');
const userData = join(smokeRoot, 'user-data');
const mediaDir = join(smokeRoot, 'media');
const marker = '[newamp-ui-handoff-smoke] ';
...
await createFixture('01 - Handoff First.mp3', 'Handoff First', 440, 4.2, 1);
await createFixture('02 - Handoff Second.mp3', 'Handoff Second', 659.25, 4.2, 2);
...
function runElectronSmoke() {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const timeout = setTimeout(() => {
      finish(new Error(`UI handoff smoke timed out without result marker. stderr:\n${tail(stderr)}`));
    }, 30000);
    ...
    child.on('exit', (code) => {
      if (!settled && code !== 0) {
        finish(new Error(`Electron exited ${code ?? 'without code'} before UI handoff result.\nstderr:\n${tail(stderr)}`));
      }
    });
