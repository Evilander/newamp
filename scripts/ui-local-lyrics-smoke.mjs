...
const smokeRoot = resolve('tmp', 'ui-local-lyrics-smoke');
...
const sidecarPath = join(mediaDir, 'Radiohead - Creep.lrc');
...
async function writeSidecarLyrics() {
  await writeFile(
    sidecarPath,
    '[00:00.50]Local sidecar lyric one\n[00:02.00]Local sidecar lyric two\n[00:04.00]Local sidecar lyric three\n',
    'utf8',
  );
}
...
function runElectronSmoke() {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const timeout = setTimeout(() => {
      finish(new Error(`UI local lyrics smoke timed out without result marker. stderr:\n${tail(stderr)}`));
    }, 45000);
    ...
    child.on('exit', (code) => {
      if (!settled && code !== 0) {
        finish(new Error(`Electron exited ${code ?? 'without code'} before UI local lyrics result.\nstderr:\n${tail(stderr)}`));
      }
    });
