...
const smokeRoot = resolve('tmp', 'ui-quick-play-smoke');
const userData = join(smokeRoot, 'user-data');
const mediaDir = join(smokeRoot, 'media');
const fixturePath = join(mediaDir, 'Newamp UI Quick Play Smoke.mp3');
const marker = '[newamp-ui-quick-play-smoke] ';
...
function runElectronSmoke() {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const timeout = setTimeout(() => {
      finish(new Error(`UI Quick Play smoke timed out without result marker. stderr:\n${tail(stderr)}`));
    }, 30000);
    ...
    child.on('exit', (code) => {
      if (!settled && code !== 0) {
        finish(new Error(`Electron exited ${code ?? 'without code'} before UI Quick Play result.\nstderr:\n${tail(stderr)}`));
      }
    });
